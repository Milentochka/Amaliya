  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-cream-100 via-cream-50 to-white px-12 py-10">
      <header className="text-center">
        ...
        <h1 className="mt-3 text-5xl font-light leading-tight text-mocha-900">
          {q.text}
        </h1>
      </header>
        ...
        {q.options.map((o, i) => {
          ...
          return (
            <div
              key={i}
              className={
                "flex items-center gap-4 rounded-3xl border px-6 py-5 text-2xl shadow-gentle transition " +
                (correct
      ...
      {revealed && q.first_correct_name && (
        <section className="mx-auto mt-8 max-w-2xl rounded-3xl border border-blush-200 bg-blush-100/40 p-6 text-center">
          <p className="text-sm uppercase tracking-widest text-blush-700">
            первым угадал
          </p>
          <p className="mt-2 text-4xl font-medium text-mocha-900">
      ...
      <section className="mx-auto mt-10 max-w-2xl rounded-3xl border border-cream-200 bg-white/80 p-6 shadow-soft">
        <ul className="divide-y divide-cream-100">
          {data.leaderboard.map((r, i) => (
            <li
              key={r.name}
              className="flex items-center justify-between py-3 text-2xl"
            >
